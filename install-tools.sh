...
tar zxvf ${TEMP_DIR}/k9s.tar.gz -C ${TARGET_PATH} k9s
...
tar zxvf ${TEMP_DIR}/kubens.tar.gz -C ${TARGET_PATH} kubens
tar zxvf ${TEMP_DIR}/kubectx.tar.gz -C ${TARGET_PATH} kubectx
tar xzvf ${TEMP_DIR}/kubecm.tar.gz -C ${TARGET_PATH} kubecm
# vault
...
tar zxvf ${TEMP_DIR}/npc/linux_${TARGETARCH}_client.tar.gz -C ${TEMP_DIR}/npc
...
tar xzvf ${TEMP_DIR}/docker-${DOCKER_VERSION}.tgz --strip 1 -C ${TARGET_PATH} docker/docker
# 安装 kind
...
tar zxvf ${TEMP_DIR}/helmfile_${HELMFILE_VERSION}_linux_${TARGETARCH}.tar.gz -C ${TARGET_PATH} helmfile
# 安装 sops
